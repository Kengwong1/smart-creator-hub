...
# ฟังก์ชันแปลและแต่ง Prompt (แก้ไข: เพิ่ม error handling)
def translate_visual(text):
    ...
    try:
        ...
        return text + ", professional photography, realistic, 8k"

# ฟังก์ชันคิดเนื้อหาภาษาไทย (แก้ไข: เพิ่ม error handling)
...
# --- 4. ฟังก์ชันสร้าง URL รูปภาพ (แก้ไข: เพิ่ม enhance และทำความสะอาด prompt) ---
def get_img_url(prompt, width, height, style_suffix):
    # รวม prompt + style และทำความสะอาด
    full_prompt = (prompt + style_suffix).replace('\n', ' ').replace('  ', ' ').strip()
    
    # Encode อย่างปลอดภัย
    encoded = urllib.parse.quote(full_prompt, safe='')
    
    # ใช้ Seed ใหม่ทุกครั้งเพื่อให้ภาพไม่ซ้ำ
    seed = int(time.time())
    
    # เพิ่ม enhance=true เพื่อคุณภาพดีขึ้น
        ...
        if not topic:
            ...
            with st.spinner("⏳ กำลังเสกเนื้อหาและรูปภาพระดับโปร..."):
                ...
                if text_res == "QUOTA_FULL":
                    ...
                    # แสดง URL สำหรับ Debug (เพิ่มใหม่)
                    with st.expander("🔧 Debug: ดู URL ภาพ"):
                        st.code(img_url)
                        st.caption("ถ้าภาพไม่แสดง ลองคลิก URL ด้านบนเพื่อเปิดดูตรง")
...
# --- 6.2 เสกรูปภาพอย่างเดียว ---
    ...
    if st.button("✨ เริ่มวาดรูป"):
        with st.spinner("🎨 กำลังบรรเลงศิลปะ..."):
            ...
            # แสดง Prompt ที่ใช้ (เพิ่มใหม่)
            with st.expander("🔍 ดู Prompt ที่ใช้"):
